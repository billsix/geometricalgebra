...
class Vector1(AbstractMultiVector):
    """An element of the grade-1 (vector) part
    of 𝒢₁.  Stored as the named fields: e_1.
    ...
    that span grades no graded type covers widen to G1.
    ...
    def to_blade_dict(self) -> BladeCoef:
        return {blade: coef for blade, coef in (((1,), self.e_1),) if coef != 0}
    ...
    def _geometric_product(self, rhs) -> typing.Self:
        match rhs:
            ...
            case _:
                left: G1 = _coerce(self, G1)
                right: G1 = _coerce(rhs, G1)
                return typing.cast(typing.Self, left * right)
    ...
    def is_close(self, other) -> bool:
        if not isinstance(other, Vector1):
            return super().is_close(other)
        return bool(np.isclose(float(self.e_1), float(other.e_1), rtol=1e-5, atol=1e-5))
...
__all__ = ["G1", "Scalar", "Vector1", "zero", "one", "e_1"]
