...
`sympy.cse`, and emitting one module per algebra. They are **not checked into git** — generate them with
...
- `make dist` regenerates, then `python -m build`s an sdist + wheel with the generated `.py` **baked
  in** — so a `pip install` is fully readable without the end user running the generator. A `build_py`
  hook in `setup.py` also regenerates *if missing* during any build (belt-and-suspenders; needs the
  `numpy`+`sympy` build-requires in `pyproject.toml`). `make upload` / `make release` push to PyPI
  (`release` refuses if a `v<version>` tag already exists — bump `version` in `pyproject.toml` first).
