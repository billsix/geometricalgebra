...
def test_orthogonal_frame_2d_symbolic() -> None:
    """Orthogonalize a general 2D frame ``a = a_1 e_1 + a_2 e_2``,
    ``b = b_1 e_1 + b_2 e_2``: the first vector is kept, the second becomes
    orthogonal to it, and the pair still spans the plane."""
    a_1, a_2, b_1, b_2 = sympy.symbols("a_1 a_2 b_1 b_2")
    a: Gn = a_1 * e_1 + a_2 * e_2
    b: Gn = b_1 * e_1 + b_2 * e_2

    w: list[MultiVectorBase] = make_orthogonal_frame([a, b])
    assert w[0] == a  # first vector unchanged
...
def test_hestenes_equals_rejection_2d_symbolic() -> None:
    """On a general 2D frame, Hestenes' ``c_k`` equals the rejection ``w_k`` scaled
    by ``|A_{k-1}|²``: ``c_1 = w_1 = a`` and ``c_2 = |a|² · w_2``.
    ...
    w: list[MultiVectorBase] = make_orthogonal_frame([a, b])
    c: list[MultiVectorBase] = make_orthogonal_frame_hestenes([a, b])
    assert c[0] == a  # c_1 = w_1 = a
    factor: object = content([a]) ** 2  # |A_1|² = |a|²
    assert c[1] - factor * w[1] == type(c[1]).zero()  # c_2 = |a|² · w_2
