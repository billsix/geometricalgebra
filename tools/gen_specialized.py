...
def scalar_doc(n: int) -> str:
    ...
    return (
        ...
        "    product results (e.g. Bivector2 * Bivector2) land here.  One type per\n"
...
def scalar_spec(n: int) -> TypeSpec:
    """The grade-0 (``Scalar``) type of 𝒢ₙ — ``Scalar1``/``Scalar2``/``Scalar3``.

    Per-algebra (not one shared type) so its dual is precise: grade 0 -> grade n
    is that algebra's pseudoscalar (``Scalar1.dual -> Vector1``, ``Scalar2 ->
    Bivector2``, ``Scalar3 -> Trivector3``), which lives in the same module.  Its
    ``dim`` is n (the shared ``Scalar`` was dimensionless, dim 0)."""
    return TypeSpec(f"Scalar{n}", ((),), n, "scalar")
...
def generate_scalar(n: int, name: str, full_name: str) -> list[ast.stmt]:
    ...
    One per algebra (``Scalar1``/``Scalar2``/``Scalar3``), emitted into that
    algebra's ``gN.py`` -- so ``dual`` can name the same-module pseudoscalar type
    (``Scalar3.dual -> Trivector3``) without a circular import.  ``full_name`` is
    the algebra's full class (``G1``/``G2``/``G3``), needed by ``unary_result`` to
    ...
    # symbolically (Scalar1.dual -> Vector1, Scalar2 -> Bivector2, Scalar3 ->
    # Trivector3).  Declared precisely (no unsound Self cast), mirroring the graded
    # types' ``dual_method``.
    dual_spec: TypeSpec
                ...
                ast.If(
                    ...
                    [
                        ast.Expr(
                            ast.Yield(
                                construct(
                                    name, [(field_name(()), self_scalar_field)]
                                )
                            )
                        )
                    ],
...
def main() -> None:
    ...
    for n, name, filename in ALGEBRAS:
        # ScalarN first (grade 0), then the full G_n, then the graded types (which
        # construct ScalarN results).  All in the one module, so ScalarN.dual can
        # name the pseudoscalar type with no cross-module (circular) import.
        nodes: list[ast.stmt] = generate_scalar(n, f"Scalar{n}", name)
